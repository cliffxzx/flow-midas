...
namespace MIDAS
{
  struct FilteringCore
  {
    ...
    FilteringCore(int numRow, int numColumn, unsigned seed, float threshold, float factor = 0.5)
        : threshold(threshold),
          factor(factor),
          ...
          numCurrentEdge(numRow, numColumn, seed),
          numTotalEdge(numCurrentEdge),
          scoreEdge(numCurrentEdge),
          numCurrentSource(numRow, numColumn, seed),
          numTotalSource(numCurrentSource),
          scoreSource(numCurrentSource),
          numCurrentDestination(numRow, numColumn, seed),
